...
# presales-quote — Devis modulaire du projet
...
- déposer le master dans `skills/sales/presales-quote/masters/` (ne pas modifier le master original)
...
Note importante : la skill `presales-quote` est conçue pour être invoquée par conversation/prompt (UX habituel). Le script `fill_devis.js` (ou le script générique `scripts/fill_master.js`) est un utilitaire local destiné aux tests et aux intégrations automatisées côté développeur — ce n'est pas le canal principal d'usage pour un utilisateur final.
