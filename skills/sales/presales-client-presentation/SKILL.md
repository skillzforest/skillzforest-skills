---
name: presales-client-presentation
description: >-
  Assemble la présentation client d'un projet (Google Slides) à partir du master de 28 slides : copie le master dans 05_Présentation client, puis renseigne les placeholders en agrégeant les livrables du projet (cadrage fonctionnel, cadrage plateforme, devis, parcours Miro, démo). Le deck résume le projet — executive summary, compréhension du besoin, vision produit, personas, périmètre, MVP, roadmap, planning, budget, TMA, risques — avec des slides CORE (toujours présentes) et OPTIONAL (à inclure selon le projet). Déclenche systématiquement ce skill dès que l'utilisateur veut produire la présentation client, le deck de proposition, ou assembler la soutenance d'avant-vente — « fais la présentation de X », « le deck client pour X », « assemble la proposition de X ». Suite Avant-Vente SkillzForest (préfixe presales-).
...
# presales-client-presentation — Présentation client (Google Slides)
