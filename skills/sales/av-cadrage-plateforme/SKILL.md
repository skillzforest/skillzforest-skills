...
- **Ne jamais inventer de chiffres.** Les coûts sont des **estimations** ; à défaut de valeur, laisser les placeholders `[€ …]` / `[… j/h]`. Ils seront consolidés avec les devis hébergeur et fournisseurs (et avec le futur skill `av-devis`).
...
Les images des schémas **logique** et **physique** (produites par `av-schema-logique` / `av-schema-physique`, exportées depuis Excalidraw) ne peuvent pas être insérées automatiquement dans le Google Doc. **Indiquer à l'utilisateur que c'est à lui d'insérer les deux images** aux emplacements `▢` des sections 3 et 5.

## Garde-fous

- **Périmètre projet strict** ; **décisions actées non ré-arbitrées** ; **aucun chiffre inventé**.
- **Cohérence chaîne** : les composants serveurs et services tiers alimenteront le devis (`av-devis`).
