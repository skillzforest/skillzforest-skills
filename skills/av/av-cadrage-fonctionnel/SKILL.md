...
- **Parcours utilisateurs** — résumer en une phrase les parcours par persona. Ce skill **ne construit pas** le mapping : le user story mapping est produit sur Miro par le skill `av-parcours-miro`. Renseigner le lien du board s'il existe, sinon laisser `[Lien_board]`.
...
Si le projet n'existe pas dans Drive, proposer d'abord `av-init-projet`, ou livrer le `.docx`.
