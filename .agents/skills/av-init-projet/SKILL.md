...
> Le master **Compte rendu** est copié une fois à l'init comme CR de base du projet. Le skill `av-compte-rendu-reunion` crée ensuite un CR par réunion (nommé avec la date) dans le même dossier — pas de collision. Les masters **Devis** et **Présentation client** seront ajoutés à ce manifeste quand ils existeront (copie dans `04_Devis` et `05_Présentation client`).
